...
### Monetary correction by IPCA, INPC, IGP-M, SELIC and TR, for AI

Automatic **monetary correction** for AI. Update any amount by IPCA, INPC, IGP-M, SELIC, TR and other official indices, with interest, penalty and attorney fees. Indices fetched live from BACEN and IBGE, no login.

- 💰 **Corrects any amount** between two dates by the chosen index
...
**Manual** (if the deeplink does not open): [claude.ai/customize/connectors](https://claude.ai/customize/connectors?surface=cowork) → **+** → **Add custom connector** → paste **Name** `Atualização Monetária` and **URL** `https://api.mcp.ai/p_atualizacao-monetaria`.

### Cursor

[➕ Install Atualização Monetária in Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=atualizacaomonetaria&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9hdHVhbGl6YWNhby1tb25ldGFyaWEifQ==)

### VS Code (Copilot Chat)

[➕ Install Atualização Monetária in VS Code](vscode:mcp/install?name=atualizacaomonetaria&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_atualizacao-monetaria%22%7D)
...
```
Update R$10,000 from 10/03/2024 to today by INPC with 1% monthly interest
What is the IGP-M correction factor between January and December 2025?
```
...
- 📧 [atualizacaomonetaria@mcp.ai](mailto:atualizacaomonetaria@mcp.ai)
- 🐛 [GitHub Issues](https://github.com/mcp-dir/atualizacao-monetaria-mcp/issues)
- 📄 [docs/](docs/)
...
MIT — see [LICENSE](LICENSE). The MCP server at `api.mcp.ai/p_atualizacao-monetaria` is proprietary (hosted); this repo (manifests, docs, skills) is MIT.
