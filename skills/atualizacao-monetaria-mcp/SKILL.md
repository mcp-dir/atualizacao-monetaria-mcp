---
name: atualizacao-monetaria-mcp
description: Atualização monetária brasileira: corrija valores por IPCA, INPC, IGP-M, SELIC, TR e outros índices oficiais, com juros, multa e honorários. Use quando o usuário pedir para atualizar, corrigir ou trazer a valor presente uma quantia entre datas.
---

# Atualização Monetária — REST API skill

Você tem acesso à **Atualização Monetária** REST API na MCP.AI.
...
Este MCP também funciona via **conexão MCP** (Claude / Cursor) em `https://api.mcp.ai/p_atualizacao-monetaria` — veja o [README](../../README.md). A skill acima é pra consumir a **REST API** direto (agente próprio / código).
